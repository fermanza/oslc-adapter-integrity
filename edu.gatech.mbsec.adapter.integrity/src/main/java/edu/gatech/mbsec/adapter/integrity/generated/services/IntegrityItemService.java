...
@OslcService(Constants.INTEGRITY_ITEM_DOMAIN)
@Path("{projectID}/items")
public class IntegrityItemService {
	@Context
	...
	@OslcQueryCapability(title = "Integrity Item Query Capability",label = "Integrity Item Catalog Query",resourceShape = OslcConstants.PATH_RESOURCE_SHAPES+ "/" + Constants.PATH_INTEGRITY_ITEM,resourceTypes = { Constants.TYPE_INTEGRITY_ITEM },usages = { OslcConstants.OSLC_USAGE_DEFAULT })
	@GET
	@Produces({ OslcMediaType.APPLICATION_RDF_XML, OslcMediaType.APPLICATION_XML, OslcMediaType.APPLICATION_JSON })
	public List<IntegrityItem> getItems(
		...
		List<IntegrityItem> oslcResources = IntegrityServerResources.getItemsInProject(projectID);
		return oslcResources;
	}
	@GET
	@Produces(MediaType.TEXT_HTML)
	public void getHtmlItems(@PathParam("projectID") final String projectID) {
		while(!IntegrityManager.serverResourcesReadFlag.getValue()){
			System.out.println("WAITING FOR ADAPTER TO FINISH SCANNING INTEGRITY SERVER");
			try {
				Thread.sleep(3000);
		...
		if (oslcResources != null) {
			...
			httpServletRequest.setAttribute("itemTypeName", "Item");
	...
	public Response getItem(
		...
		IntegrityItem oslcResource = IntegrityServerResources.getItemByURI(projectID + "/items/" + ID);
	...
	public void getHtmlItem(
		...
		for (Link fieldLink : oslcResource.getField()) {URI fieldURI = fieldLink.getValue();
			IntegrityField integrityField = IntegrityUtil.getFieldByURI(fieldURI);
			String fieldName = integrityField.getName();
			String fieldValue = integrityField.getValue();
			fieldNameValueMap.put(fieldName, fieldValue);
		}
			httpServletRequest.setAttribute("fieldNameValueMap", fieldNameValueMap);
			httpServletRequest.setAttribute("bugzillaUri","https://landfill.bugzilla.org/bugzilla-4.2-branch/");
			httpServletRequest.setAttribute("element", oslcResource);
			httpServletRequest.setAttribute("requestURL", requestURL);
			httpServletRequest.setAttribute("projectName", projectName);
			RequestDispatcher rd = httpServletRequest.getRequestDispatcher("/integrity/item_html.jsp");
			try {
				rd.forward(httpServletRequest, httpServletResponse);
			} catch (Exception e) {
				e.printStackTrace();
