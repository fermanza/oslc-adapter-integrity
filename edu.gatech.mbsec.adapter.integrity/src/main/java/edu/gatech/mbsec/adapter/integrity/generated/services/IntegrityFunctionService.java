...
@OslcService(Constants.INTEGRITY_FUNCTION_DOMAIN)
@Path("{projectID}/functions")
public class IntegrityFunctionService {
	@Context
	...
	@OslcQueryCapability(title = "Integrity Function Query Capability",label = "Integrity Function Catalog Query",resourceShape = OslcConstants.PATH_RESOURCE_SHAPES+ "/" + Constants.PATH_INTEGRITY_FUNCTION,resourceTypes = { Constants.TYPE_INTEGRITY_FUNCTION },usages = { OslcConstants.OSLC_USAGE_DEFAULT })
	@GET
	@Produces({ OslcMediaType.APPLICATION_RDF_XML, OslcMediaType.APPLICATION_XML, OslcMediaType.APPLICATION_JSON })
	public List<IntegrityFunction> getFunctions(
		...
		List<IntegrityFunction> oslcResources = IntegrityServerResources.getFunctionsInProject(projectID);
		return oslcResources;
	}
	@GET
	@Produces(MediaType.TEXT_HTML)
	public void getHtmlFunctions(@PathParam("projectID") final String projectID) {
		while(!IntegrityManager.serverResourcesReadFlag.getValue()){
			System.out.println("WAITING FOR ADAPTER TO FINISH SCANNING INTEGRITY SERVER");
			try {
				Thread.sleep(3000);
		...
		if (oslcResources != null) {
			...
			httpServletRequest.setAttribute("itemTypeName", "Function");
	...
	public Response getFunction(
		...
		IntegrityFunction oslcResource = IntegrityServerResources.getFunctionByURI(projectID + "/functions/" + ID);
