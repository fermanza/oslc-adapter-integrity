...
@OslcService(Constants.INTEGRITY_FIELD_DOMAIN)
@Path("{projectID}/fields")
public class IntegrityFieldService {
	@Context
	...
	@OslcQueryCapability(title = "Integrity Field Query Capability",label = "Integrity Field Catalog Query",resourceShape = OslcConstants.PATH_RESOURCE_SHAPES+ "/" + Constants.PATH_INTEGRITY_FIELD,resourceTypes = { Constants.TYPE_INTEGRITY_FIELD },usages = { OslcConstants.OSLC_USAGE_DEFAULT })
	@GET
	@Produces({ OslcMediaType.APPLICATION_RDF_XML, OslcMediaType.APPLICATION_XML, OslcMediaType.APPLICATION_JSON })
	public List<IntegrityField> getFields(
		...
		List<IntegrityField> oslcResources = IntegrityServerResources.getFieldsInProject(projectID);
		return oslcResources;
	}
	@GET
	@Produces(MediaType.TEXT_HTML)
	public void getHtmlFields(@PathParam("projectID") final String projectID) {
		while(!IntegrityManager.serverResourcesReadFlag.getValue()){
			System.out.println("WAITING FOR ADAPTER TO FINISH SCANNING INTEGRITY SERVER");
			try {
				Thread.sleep(3000);
		...
		if (oslcResources != null) {
			...
			httpServletRequest.setAttribute("itemTypeName", "Field");
	...
	public Response getField(
		...
		IntegrityField oslcResource = IntegrityServerResources.getFieldByURI(projectID + "/fields/" + ID);
