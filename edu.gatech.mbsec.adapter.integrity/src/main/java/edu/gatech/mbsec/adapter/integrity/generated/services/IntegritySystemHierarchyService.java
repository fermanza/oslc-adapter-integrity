...
@OslcService(Constants.INTEGRITY_SYSTEMHIERARCHY_DOMAIN)
@Path("{projectID}/systemhierarchys")
public class IntegritySystemHierarchyService {
	@Context
	...
	@OslcQueryCapability(title = "Integrity SystemHierarchy Query Capability",label = "Integrity SystemHierarchy Catalog Query",resourceShape = OslcConstants.PATH_RESOURCE_SHAPES+ "/" + Constants.PATH_INTEGRITY_SYSTEMHIERARCHY,resourceTypes = { Constants.TYPE_INTEGRITY_SYSTEMHIERARCHY },usages = { OslcConstants.OSLC_USAGE_DEFAULT })
	@GET
	@Produces({ OslcMediaType.APPLICATION_RDF_XML, OslcMediaType.APPLICATION_XML, OslcMediaType.APPLICATION_JSON })
	public List<IntegritySystemHierarchy> getSystemHierarchys(
		...
		List<IntegritySystemHierarchy> oslcResources = IntegrityServerResources.getSystemHierarchysInProject(projectID);
		return oslcResources;
	}
	@GET
	@Produces(MediaType.TEXT_HTML)
	public void getHtmlSystemHierarchys(@PathParam("projectID") final String projectID) {
		while(!IntegrityManager.serverResourcesReadFlag.getValue()){
			System.out.println("WAITING FOR ADAPTER TO FINISH SCANNING INTEGRITY SERVER");
			try {
				Thread.sleep(3000);
		...
		if (oslcResources != null) {
			...
			httpServletRequest.setAttribute("itemTypeName", "SystemHierarchy");
	...
	@Path("{uri}")
	@Produces({ OslcMediaType.APPLICATION_RDF_XML, OslcMediaType.APPLICATION_XML, OslcMediaType.APPLICATION_JSON })
	public Response getSystemHierarchy(
		@PathParam("projectID") final String projectID,
		...
		IntegritySystemHierarchy oslcResource = IntegrityServerResources.getSystemHierarchyByURI(projectID + "/systemhierarchys/" + ID);
	...
	@Path("{uri}")
	@Produces(MediaType.TEXT_HTML)
	public void getHtmlSystemHierarchy(
		@PathParam("projectID") final String projectID,
		@PathParam("uri") final String ID)
