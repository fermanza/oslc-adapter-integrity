...
@OslcService(Constants.INTEGRITY_FUNCTIONHIERARCHY_DOMAIN)
@Path("{projectID}/functionhierarchys")
public class IntegrityFunctionHierarchyService {
	@Context
	...
	@OslcQueryCapability(title = "Integrity FunctionHierarchy Query Capability",label = "Integrity FunctionHierarchy Catalog Query",resourceShape = OslcConstants.PATH_RESOURCE_SHAPES+ "/" + Constants.PATH_INTEGRITY_FUNCTIONHIERARCHY,resourceTypes = { Constants.TYPE_INTEGRITY_FUNCTIONHIERARCHY },usages = { OslcConstants.OSLC_USAGE_DEFAULT })
	@GET
	@Produces({ OslcMediaType.APPLICATION_RDF_XML, OslcMediaType.APPLICATION_XML, OslcMediaType.APPLICATION_JSON })
	public List<IntegrityFunctionHierarchy> getFunctionHierarchys(
		...
		List<IntegrityFunctionHierarchy> oslcResources = IntegrityServerResources.getFunctionHierarchysInProject(projectID);
		return oslcResources;
	}
	@GET
	@Produces(MediaType.TEXT_HTML)
	public void getHtmlFunctionHierarchys(@PathParam("projectID") final String projectID) {
		while(!IntegrityManager.serverResourcesReadFlag.getValue()){
			System.out.println("WAITING FOR ADAPTER TO FINISH SCANNING INTEGRITY SERVER");
			try {
				Thread.sleep(3000);
		...
		if (oslcResources != null) {
			...
			httpServletRequest.setAttribute("itemTypeName", "FunctionHierarchy");
	...
	@Path("{uri}")
	@Produces({ OslcMediaType.APPLICATION_RDF_XML, OslcMediaType.APPLICATION_XML, OslcMediaType.APPLICATION_JSON })
	public Response getFunctionHierarchy(
		@PathParam("projectID") final String projectID,
		...
		IntegrityFunctionHierarchy oslcResource = IntegrityServerResources.getFunctionHierarchyByURI(projectID + "/functionhierarchys/" + ID);
	...
	@Path("{uri}")
	@Produces(MediaType.TEXT_HTML)
	public void getHtmlFunctionHierarchy(
		@PathParam("projectID") final String projectID,
		@PathParam("uri") final String ID)
