...
@OslcService(Constants.INTEGRITY_SERVER_DOMAIN)
@Path("{projectID}/servers")
public class IntegrityServerService {
	@Context
	...
	@OslcQueryCapability(title = "Integrity Server Query Capability",label = "Integrity Server Catalog Query",resourceShape = OslcConstants.PATH_RESOURCE_SHAPES+ "/" + Constants.PATH_INTEGRITY_SERVER,resourceTypes = { Constants.TYPE_INTEGRITY_SERVER },usages = { OslcConstants.OSLC_USAGE_DEFAULT })
	@GET
	@Produces({ OslcMediaType.APPLICATION_RDF_XML, OslcMediaType.APPLICATION_XML, OslcMediaType.APPLICATION_JSON })
	public List<IntegrityServer> getServers(
		...
		List<IntegrityServer> oslcResources = IntegrityServerResources.getServersInProject(projectID);
		return oslcResources;
	}
	@GET
	@Produces(MediaType.TEXT_HTML)
	public void getHtmlServers(@PathParam("projectID") final String projectID) {
		while(!IntegrityManager.serverResourcesReadFlag.getValue()){
			System.out.println("WAITING FOR ADAPTER TO FINISH SCANNING INTEGRITY SERVER");
			try {
				Thread.sleep(3000);
		...
		if (oslcResources != null) {
			...
			httpServletRequest.setAttribute("itemTypeName", "Server");
	...
	public Response getServer(
		...
		IntegrityServer oslcResource = IntegrityServerResources.getServerByURI(projectID + "/servers/" + ID);
